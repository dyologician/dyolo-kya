...
// Domain constants must match dyolo::kya exactly — these are version-pinned.
const DOMAIN_CERT_SIG:    &str = "dyolo::kya::cert::sig::v1";
const DOMAIN_CERT_FP:     &str = "dyolo::kya::cert::fp::v1";
const DOMAIN_CHAIN_FP:    &str = "dyolo::kya::chain::fp::v1";
const DOMAIN_SUBSCOPE:    &str = "dyolo::kya::subscope::commit::v1";
const DOMAIN_MERKLE_NODE: &str = "dyolo::kya::merkle::node::v1";
...
pub fn main() {
    ...
    for (i, cert) in certs.iter().enumerate() {
        ...
        assert!(
            ...
            "dyolo::kya::zk: broken linkage at hop {i}"
        );

        // Temporal monotonicity: child cannot outlive parent.
        assert!(
            cert.expiration_unix <= parent_expiry,
            "dyolo::kya::zk: temporal violation at hop {i}"
        );

        // Temporal validity at the attested verification timestamp.
        assert!(cert.issued_at <= verified_at, "dyolo::kya::zk: cert not yet valid at hop {i}");
        assert!(cert.expiration_unix >= verified_at, "dyolo::kya::zk: cert expired at hop {i}");

        // Depth cap enforcement.
        let depth = (i + 1) as u8;
        assert!(depth <= max_allowed_depth, "dyolo::kya::zk: max depth exceeded at hop {i}");
        ...
        if cert.subset_intents.is_empty() {
            assert!(
                ...
                "dyolo::kya::zk: scope escalation at hop {i}"
            );
        } else {
            for (intent_hash, proof_nodes) in cert.subset_intents.iter().zip(cert.proofs.iter()) {
                ...
                assert!(
                    ...
                    "dyolo::kya::zk: invalid subscope proof at hop {i}"
                );
            }
            ...
            assert!(
                ...
                "dyolo::kya::zk: derived scope root mismatch at hop {i}"
            );
        }
        ...
        let delegator_vk = VerifyingKey::from_bytes(&cert.delegator_pk)
            .expect("dyolo::kya::zk: invalid delegator key");
        ...
        delegator_vk.verify(&msg, &sig)
            .unwrap_or_else(|_| panic!("dyolo::kya::zk: invalid signature at hop {i}"));
    ...
    assert!(
        ...
        "dyolo::kya::zk: unauthorized leaf"
    );
    ...
    assert!(
        ...
        "dyolo::kya::zk: chain fingerprint mismatch"
    );
...
// ── Internal helpers — mirrors dyolo-kya internals exactly ───────────────────
