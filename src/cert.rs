...
impl DelegationCert {
    ...
    pub(crate) fn issue(
        ...
        let ext_commit = extensions_hash.unwrap_or_else(|| {
            let mut h = crate::crypto::derive_key("dyolo::kya::cert::ext::v1", CERT_VERSION);
            h.update(&0u64.to_le_bytes());
            h.finalize().into()
        });

        let msg = Self::signable_bytes(
            CERT_VERSION, &delegator_pk, &delegate_pk, &scope_root,
            &scope_proof, &nonce, issued_at, expiration_unix, max_depth, &ext_commit,
        );
        Self {
    ...
    pub(crate) fn verify_signature(&self) -> bool {
        ...
        let ext_commit = self.extensions_hash.unwrap_or_else(|| {
            let mut h = crate::crypto::derive_key("dyolo::kya::cert::ext::v1", self.version);
            h.update(&0u64.to_le_bytes());
            h.finalize().into()
        });
        
        let msg = Self::signable_bytes(
            self.version, &self.delegator_pk, &self.delegate_pk, &self.scope_root,
            &self.scope_proof, &self.nonce, self.issued_at, self.expiration_unix,
            self.max_depth, &ext_commit,
        );
...
impl CertBuilder {
    ...
    pub fn build(self, delegator: &dyn Signer) -> Result<DelegationCert, KyaError> {
        if self.issued_at >= self.expiration_unix {
            return Err(KyaError::WireFormatError(
                format!("issued_at ({}) must be strictly less than expiration_unix ({})", self.issued_at, self.expiration_unix)
            ));
        }
    ...
    pub fn sign(self, delegator: &dyn Signer) -> DelegationCert {
        self.build(delegator).expect("invalid certificate configuration: issued_at must be before expiration_unix")
    }

    #[cfg(feature = "async")]
    #[cfg_attr(docsrs, doc(cfg(feature = "async")))]
    pub async fn build_async(self, delegator: &dyn crate::identity::AsyncSigner) -> Result<DelegationCert, KyaError> {
        if self.issued_at >= self.expiration_unix {
            return Err(KyaError::WireFormatError(
                format!("issued_at ({}) must be strictly less than expiration_unix ({})", self.issued_at, self.expiration_unix)
            ));
        ...
        let msg = DelegationCert::signable_bytes(
            CERT_VERSION, &delegator_pk, &self.delegate_pk, &self.scope_root,
            &self.scope_proof, &self.nonce, self.issued_at, self.expiration_unix, self.max_depth, &ext_commit,
        );
    ...
    pub async fn sign_async(self, delegator: &dyn crate::identity::AsyncSigner) -> DelegationCert {
        self.build_async(delegator).await.expect("invalid certificate configuration: issued_at must be before expiration_unix")
    }
}
