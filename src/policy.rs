...
impl std::fmt::Display for PolicyViolation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::TtlExceedsMaximum { allowed_secs, actual_secs } =>
                write!(f, "cert TTL {actual_secs}s exceeds policy maximum {allowed_secs}s"),
...
///
/// ```rust
/// use dyolo_kya::policy::CapabilitySet;
///
...
///
/// ```rust
/// use dyolo_kya::policy::{DelegationPolicy, CapabilitySet};
///
...
impl DelegationPolicy {
    ...
    pub fn check_cert(&self, cert: &DelegationCert) -> Result<(), PolicyViolation> {
        ...
        if !self.allow_sub_delegation {
            let has_sub = !cert.scope_proof.subset_intents.is_empty()
...
impl PolicySet {
    ...
    pub fn check_chain(&self, chain: &DyoloChain) -> Result<(), KyaError> {
        for policy in &self.policies {
            policy.check_chain(chain).map_err(|v| KyaError::PolicyViolation(v.to_string()))?;
        }
        Ok(())
    }

    pub fn check_intent(&self, intent: &Intent) -> Result<(), KyaError> {
        for policy in &self.policies {
            policy.check_intent(intent).map_err(|v| KyaError::PolicyViolation(v.to_string()))?;
        }
        Ok(())
    }
    ...
    pub fn from_yaml(yaml: &str) -> Result<Self, serde_yaml::Error> {
        // We define a shadow struct to derive deserialize, allowing us to parse
        // either a single policy or a list of policies.
...
mod tests {
    ...
    use crate::{
        ...
        registry::fresh_nonce,
    };

    fn make_chain(depth: usize, ttl: u64) -> DyoloChain {
        let root  = DyoloIdentity::generate();
        let scope = IntentTree::build(vec![intent_hash("trade.equity", b"")]).unwrap().root();
    ...
    fn intent_checked_against_capabilities() {
        let policy = DelegationPolicy::new("test")
