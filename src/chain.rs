...
impl AuthorizedAction {
    ...
    pub fn receipt(&self) -> &VerificationReceipt { &self.receipt }
}

impl std::fmt::Debug for AuthorizedAction {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AuthorizedAction").field("receipt", &self.receipt).finish()
    }
}
...
impl DyoloChain {
    ...
    pub fn certs(&self) -> &[DelegationCert] { &self.certs }
    ...
    pub fn fingerprint(&self) -> [u8; 32] {
        ...
        self.certs.iter().fold(h, |mut h, cert| { h.update(&cert.fingerprint()); h })
            .finalize().into()
    }
    ...
    fn validate_structure(
        ...
        if self.certs.is_empty() { return Err(KyaError::EmptyChain); }
        if self.certs[0].delegator_pk != self.principal_pk { return Err(KyaError::RootMismatch); }
        ...
        if chain_len > 255 { return Err(KyaError::MaxDepthExceeded(255, 255)); }
        ...
        for (i, cert) in self.certs.iter().enumerate() {
            if cert.delegator_pk != expected_delegator { return Err(KyaError::BrokenLinkage(i)); }

            if cert.version != crate::cert::CERT_VERSION {
                return Err(KyaError::UnsupportedVersion {
                    expected: crate::cert::CERT_VERSION, got: cert.version,
                });
            }
            ...
            batch_msgs.push(DelegationCert::signable_bytes(
                cert.version, &cert.delegator_pk, &cert.delegate_pk, &cert.scope_root,
                &cert.scope_proof, &cert.nonce, cert.issued_at, cert.expiration_unix,
                cert.max_depth, &ext_commit,
            ));
            batch_sigs.push(cert.signature);
            batch_pks.push(cert.delegator_pk);

            if tolerated_early < cert.issued_at { return Err(KyaError::NotYetValid(i, cert.issued_at, now)); }
            if cert.expiration_unix < tolerated_late { return Err(KyaError::Expired(i, cert.expiration_unix, now)); }
            if cert.expiration_unix > parent_expiry { return Err(KyaError::TemporalViolation(i, cert.expiration_unix, parent_expiry)); }

            depth += 1;
            if depth > max_allowed_depth as usize { return Err(KyaError::MaxDepthExceeded(i, max_allowed_depth)); }
            if cert.max_depth < max_allowed_depth { max_allowed_depth = cert.max_depth; }

            for seen in &seen_nonces {
                if seen == &cert.nonce { return Err(KyaError::NonceReplay); }
            }
            seen_nonces.push(cert.nonce);
            cert_fingerprints.push(cert.fingerprint());

            let is_passthrough = cert.scope_proof.subset_intents.is_empty()
                && cert.scope_proof.proofs.is_empty();
            if is_passthrough {
                ...
                let derived = cert.scope_proof.verify_and_derive_root(&current_scope)
        ...
        if expected_delegator != *agent_pk { return Err(KyaError::UnauthorizedLeaf); }

        {
            ...
            if ed25519_dalek::verify_batch(&msgs_refs, &batch_sigs, &batch_pks).is_err() {
                for (i, cert) in self.certs.iter().enumerate() {
                    if !cert.verify_signature() { return Err(KyaError::InvalidSignature(i)); }
                }
        ...
        if !intent_authorized { return Err(KyaError::ScopeViolation); }
    ...
    pub fn authorize(
        ...
        self.authorize_with_options(agent_pk, intent, proof, clock, revocation, nonces, None, &NoopAuditSink)
    }
    ...
    pub fn authorize_with_options(
        ...
        #[cfg(feature = "tracing")]
        let _span = tracing::info_span!("dyolo_kya::authorize", chain_len = self.certs.len()).entered();

        let principal_hex = hex::encode(self.principal_pk.as_bytes());
        let executor_hex  = hex::encode(agent_pk.as_bytes());

        let result = self.authorize_inner(agent_pk, intent_h, proof, clock, revocation, nonces, policy);
    ...
    fn authorize_inner(
        ...
        let v = self.validate_structure(agent_pk, intent_h, proof, clock, self.drift_tolerance_secs)?;

        for fp in &v.cert_fingerprints {
            if revocation.is_revoked(fp).map_err(KyaError::StorageFailure)? {
                return Err(KyaError::Revoked);
            }
        }

        if !nonces.try_consume_batch(&v.seen_nonces).map_err(KyaError::StorageFailure)? {
    ...
    pub fn authorize_batch(
        ...
        let v = match self.validate_structure(agent_pk, first_intent, first_proof, clock, self.drift_tolerance_secs) {
            Ok(v)  => v,
            Err(e) => {
                let n   = intents.len();
                let msg = e.to_string();
                return BatchAuthorizeResult {
                    receipts:       vec![None; n],
                    errors:         (0..n).map(|i| Some(KyaError::BatchItemFailed { index: i, reason: msg.clone() })).collect(),
        ...
        for fp in &v.cert_fingerprints {
            match revocation.is_revoked(fp) {
                ...
                Err(e) => {
                    ...
                    return BatchAuthorizeResult {
                        receipts:       vec![None; n],
                        errors:         (0..n).map(|_| Some(KyaError::BatchItemFailed { index: 0, reason: msg.clone() })).collect(),
                        all_authorized: false,
                    };
                }
                Ok(false) => {}
        ...
        if !all_ok {
            return BatchAuthorizeResult { receipts, errors, all_authorized: false };
        }
        ...
        BatchAuthorizeResult { receipts, errors, all_authorized: true }
    }
    ...
    pub async fn authorize_async(
        ...
        self.authorize_async_with_options(agent_pk, intent, proof, clock, revocation, nonces, None, &NoopAuditSink).await
    }
    ...
    pub async fn authorize_async_with_options(
        ...
        #[cfg(feature = "tracing")]
        let _span = tracing::info_span!("dyolo_kya::authorize_async", chain_len = self.certs.len()).entered();
        ...
        let result = async {
            ...
            for fp in &v.cert_fingerprints {
                if revocation.is_revoked(fp).await.map_err(KyaError::StorageFailure)? {
                    return Err(KyaError::Revoked);
                }
            }

            if !nonces.try_consume_batch(&v.seen_nonces).await.map_err(KyaError::StorageFailure)? {
                return Err(KyaError::NonceReplay);
            }
...
mod tests {
    ...
    impl Clock for FixedClock { fn unix_now(&self) -> u64 { self.0 } }
    ...
    #[allow(deprecated)]
    fn two_hop_chain(
        human: &DyoloIdentity, a: &DyoloIdentity, b: &DyoloIdentity,
        scope: IntentHash, now: u64,
    ) -> DyoloChain {
        let expiry = now + 3600;
    ...
    fn full_delegation_chain_succeeds() {
        ...
        let action = chain.authorize(
            &b.verifying_key(), &trade, &proof,
            &FixedClock(now), &MemoryRevocationStore::new(), &MemoryNonceStore::new(),
        ).unwrap();
    ...
    fn namespace_isolation_different_scopes() {
        ...
        let trade = intent_hash("trade", b"");
        let tree  = IntentTree::build(vec![trade]).unwrap();
        let now   = 1_700_000_000u64;

        let chain_a = DyoloChain::new(human.verifying_key(), tree.root()).with_namespace("tenant-a");
        let chain_b = DyoloChain::new(human.verifying_key(), tree.root()).with_namespace("tenant-b");

        assert_ne!(chain_a.principal_scope, chain_b.principal_scope,
            "namespaced chains must have different effective scopes");
        assert_ne!(chain_a.fingerprint(), chain_b.fingerprint());

        let cert_a = CertBuilder::new(agent.verifying_key(), chain_a.principal_scope, now, now + 3600).sign(&human);
        let cert_b = CertBuilder::new(agent.verifying_key(), chain_b.principal_scope, now, now + 3600).sign(&human);

        let mut ca = DyoloChain::new(human.verifying_key(), tree.root()).with_namespace("tenant-a");
        ca.push(cert_a);
        let mut cb = DyoloChain::new(human.verifying_key(), tree.root()).with_namespace("tenant-b");
        cb.push(cert_b);

        let proof = tree.prove(&trade).unwrap();

        let ok_a = ca.authorize(&agent.verifying_key(), &trade, &proof, &FixedClock(now), &MemoryRevocationStore::new(), &MemoryNonceStore::new());
        let ok_b = cb.authorize(&agent.verifying_key(), &trade, &proof, &FixedClock(now), &MemoryRevocationStore::new(), &MemoryNonceStore::new());

        assert!(ok_a.is_ok(), "tenant-a chain should authorize");
        assert!(ok_a.unwrap().receipt.namespace.as_deref() == Some("tenant-a"));
        assert!(ok_b.is_ok(), "tenant-b chain should authorize independently");

        let cert_wrong = CertBuilder::new(agent.verifying_key(), chain_b.principal_scope, now, now + 3600).sign(&human);
        let mut c_wrong = DyoloChain::new(human.verifying_key(), tree.root()).with_namespace("tenant-a");
        c_wrong.push(cert_wrong);
        let result = c_wrong.authorize(&agent.verifying_key(), &trade, &proof, &FixedClock(now), &MemoryRevocationStore::new(), &MemoryNonceStore::new());
        assert!(result.is_err(), "cert issued for tenant-b must not work under tenant-a chain");
    }
    ...
    async fn authorize_async_succeeds() {
        ...
        let action = chain.authorize_async(&b.verifying_key(), &trade, &proof, &FixedClock(now), &rev, &nonces).await.unwrap();
        assert_eq!(action.receipt.chain_depth, 2);
    }
}