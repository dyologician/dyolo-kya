...
use dyolo_kya::{Intent, IntentHash, MerkleProof, SystemClock, wire::SignedChain};
...
async fn batch_inner(
...
) -> Result<BatchAuthorizeResponse, dyolo_kya::KyaError> {
    if req.intents.len() > MAX_BATCH_SIZE {
        return Err(dyolo_kya::KyaError::WireFormatError(format!(
    ...
    let pk_bytes = hex::decode(&req.executor_pk_hex)
        .map_err(|_| dyolo_kya::KyaError::WireFormatError("invalid executor_pk_hex".into()))?;
    let pk_arr: [u8; 32] = pk_bytes.try_into()
        .map_err(|_| dyolo_kya::KyaError::WireFormatError("executor_pk must be 32 bytes".into()))?;
    let executor_pk = ed25519_dalek::VerifyingKey::from_bytes(&pk_arr)
        .map_err(|_| dyolo_kya::KyaError::WireFormatError("invalid executor Ed25519 key".into()))?;

    let chain = req.chain.into_chain_with_drift(15)?;

    let mut intent_pairs: Vec<(dyolo_kya::IntentHash, MerkleProof)> = Vec::with_capacity(req.intents.len());
    ...
    let batch = chain.authorize_batch_async(
        &executor_pk,
        &intent_pairs,
        &SystemClock,
        &*state.revocation,
    ...
    let results: Vec<BatchItem> = req.intents.iter().enumerate()
        .map(|(i, intent_req)| {
