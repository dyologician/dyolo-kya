...
func TestWellKnown_success(t *testing.T) {
	...
	srv := okServer(t, "/.well-known/kya-configuration", payload)
	defer srv.Close()
	...
	if cfg.KyaVersion != "2.0.0" {
		t.Fatalf("version: got %q", cfg.KyaVersion)
	}
}
...
func TestAuthorize_gateway_error(t *testing.T) {
	...
	kyaErr, ok := err.(*KyaError)
	if !ok {
		t.Fatalf("expected *KyaError, got %T", err)
	}
	if kyaErr.ErrorCode != "CHAIN_EXPIRED" {
		t.Fatalf("error_code: got %q", kyaErr.ErrorCode)
	}
	if kyaErr.Status != http.StatusForbidden {
		t.Fatalf("status: got %d", kyaErr.Status)
	}
}
...
func TestKyaError_message_format(t *testing.T) {
	err := &KyaError{
		Message:   "forbidden",
	...
	err2 := &KyaError{Message: "generic", Status: 500}
	if err2.Error() == "" {
...
func TestAuthorize_nonJSON_500(t *testing.T) {
	...
	if kyaErr.Status != http.StatusBadGateway {
		t.Fatalf("status: got %d", kyaErr.Status)
	}
}
