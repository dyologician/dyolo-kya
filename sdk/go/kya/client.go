// Package kya is the Go client for the dyolo-kya Know Your Agent gateway.
//
// The dyolo-kya protocol provides cryptographic chain-of-custody for recursive
...
// Usage:
//
//	c := kya.New("http://localhost:8080")
//	result, err := c.Authorize(ctx, kya.AuthorizeRequest{...})
package kya
...
// Client talks to the dyolo-kya gateway over HTTP.
type Client struct {
	base    string
	http    *http.Client
...
// New returns a Client pointed at the dyolo-kya gateway at baseURL.
...
type WellKnownConfig struct {
	...
	KyaVersion               string   `json:"kya_version"`
...
func (c *Client) WellKnown(ctx context.Context) (*WellKnownConfig, error) {
	...
	if err := c.get(ctx, "/.well-known/kya-configuration", &out); err != nil {
		return nil, err
	}
	return &out, nil
}
...
// KyaError is returned when the gateway rejects a request.
...
func (e *KyaError) Error() string {
	if e.ErrorCode != "" {
		return fmt.Sprintf("dyolo-kya [%s]: %s (HTTP %d)", e.ErrorCode, e.Message, e.Status)
	}
	return fmt.Sprintf("dyolo-kya: %s (HTTP %d)", e.Message, e.Status)
}
...
func (c *Client) post(ctx context.Context, path string, body, out interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("kya: marshal request: %w", err)
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, c.base+path, bytes.NewReader(data))
	if err != nil {
		return fmt.Errorf("kya: build request: %w", err)
	}
...
func (c *Client) do(req *http.Request, out interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("kya: http: %w", err)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return fmt.Errorf("kya: read response: %w", err)
	}
	...
	if out != nil {
		if err := json.Unmarshal(body, out); err != nil {
			return fmt.Errorf("kya: decode response: %w", err)
		}
	}
	return nil
}
