import {
  buildLangChainKyaTool,
  buildLangChainKyaBatchTool,
  buildOpenAIKyaFunction,
  buildOpenAIKyaBatchFunction,
  withKyaGuard,
  withKyaBatchGuard,
...
function makeClient(overrides: Partial<KyaClient> = {}): KyaClient {
  const client = Object.create(KyaClient.prototype) as KyaClient;
  Object.assign(
    client,
    {
...
describe("buildLangChainKyaTool", () => {
  ...
  it("wraps KyaError with intent context and rethrows", async () => {
    const kyaErr = new KyaError("signature mismatch", "SIG_INVALID", 403);
    const client = makeClient({ authorize: jest.fn().mockRejectedValue(kyaErr) } as never);
  ...
  it("propagates non-KyaError exceptions unchanged", async () => {
...
describe("buildLangChainKyaBatchTool", () => {
  ...
  it("wraps KyaError from authorizeBatch", async () => {
    const kyaErr = new KyaError("chain expired", "CHAIN_EXPIRED", 403);
    const client = makeClient({ authorizeBatch: jest.fn().mockRejectedValue(kyaErr) } as never);
...
describe("buildOpenAIKyaFunction", () => {
  ...
  it("returns a correct definition block", () => {
    const fn = buildOpenAIKyaFunction<TradeArgs>({
      name: "execute_trade",
      description: "Execute a trade",
  ...
  it("returns error JSON with code when KyaError is thrown from authorize", async () => {
    const kyaErr = new KyaError("expired", "CHAIN_EXPIRED", 403);
    const client = makeClient({ authorize: jest.fn().mockRejectedValue(kyaErr) } as never);
...
describe("buildOpenAIKyaBatchFunction", () => {
  ...
  it("returns error JSON with code on KyaError from authorizeBatch", async () => {
    const kyaErr = new KyaError("revoked", "CERT_REVOKED", 403);
    const client = makeClient({ authorizeBatch: jest.fn().mockRejectedValue(kyaErr) } as never);
...
describe("withKyaGuard", () => {
  ...
  it("propagates KyaError from authorize", async () => {
    const kyaErr = new KyaError("unauthorized", "UNAUTHORIZED", 403);
    const client = makeClient({ authorize: jest.fn().mockRejectedValue(kyaErr) } as never);
    ...
    await expect(guarded({})).rejects.toBeInstanceOf(KyaError);
  });
});

// ── withKyaBatchGuard ─────────────────────────────────────────────────────────

describe("withKyaBatchGuard", () => {
  it("authorizes batch and passes result to wrapped function when all authorized", async () => {
    ...
    const guarded = withKyaBatchGuard({
      intentNames: ["query.portfolio", "trade.equity"],
      client,
      resolveContext: () => ({ chain: MOCK_CHAIN, executorPkHex: MOCK_EXECUTOR_PK }),
      fn,
  ...
  it("throws KyaError listing denied intents when batch is not fully authorized", async () => {
    const partial: BatchAuthorizeResult = {
      allAuthorized: false,
      authorizedCount: 0,
      totalCount: 2,
...
describe("KyaError", () => {
  it("carries code and status", () => {
    const err = new KyaError("bad chain", "CHAIN_INVALID", 403);
    expect(err.name).toBe("KyaError");
  ...
  it("can be constructed with optional fields omitted", () => {
    const err = new KyaError("generic");
