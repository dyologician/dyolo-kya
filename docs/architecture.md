# dyolo-kya Enterprise Architecture
...
The core problem dyolo-kya solves is the "Recursive Delegation Gap." When a human asks Agent A to perform a task, and Agent A delegates to Agent B, the authorization context is usually lost. dyolo-kya fixes this by enforcing a cryptographically verifiable chain-of-custody.
```mermaid
sequenceDiagram
    autonumber
    ...
    participant Gateway as dyolo-kya-gateway
...
Deploying `dyolo-kya-gateway` in a production environment requires strict adherence to security and high-availability (HA) best practices. This document covers the required configurations for enterprise architecture review.

## 1. Network & TLS Termination

The `dyolo-kya-gateway` runs over HTTP. It **must** be deployed behind a secure reverse proxy or service mesh that handles TLS termination.
...
*   **Configuration:** 
    ```env
    DYOLO_PG_URL="postgres://kya_user:kya_pass@pgbouncer.internal:6432/kya_db"
    ```
...
```env
    DYOLO_REDIS_URL="redis://kya_redis_primary.internal:6379/0"
    ```
...
*   **`DYOLO_SIGNING_KEY_HEX`**: The Ed25519 private key used to sign `DelegationCert` issuance requests.
*   **`DYOLO_MAC_KEY_HEX`**: The 32-byte Blake3 key used to authenticate `VerifiedToken` receipts.

**Rotation Strategy:**
1. Generate new keys using `dyolo-kya keygen`.
