...
use a1::{CertBuilder, CertExtensions, ExtValue, Intent, IntentTree};

use crate::state::AppState;

static IDEMPOTENCY_CACHE: OnceLock<Mutex<HashMap<String, (u64, IssueCertResponse)>>> =
    OnceLock::new();
static BATCH_IDEMPOTENCY_CACHE: OnceLock<Mutex<HashMap<String, (u64, IssueBatchResponse)>>> =
    OnceLock::new();
...
pub async fn issue_batch_handler(
    ...
    let mut certs: Vec<a1::DelegationCert> = Vec::with_capacity(req.requests.len());
    ...
    let bundle = a1::CertBundle {
        certs,
        issued_at: unix_now(),
    };
    let total = issued.len();
    let response = IssueBatchResponse {
        bundle,
        issued,
        total,
    };
...
pub async fn revoke_handler(
    ...
    match state.revocation.revoke(&fp).await {
        Ok(()) => (
            StatusCode::OK,
            Json(serde_json::json!({ "revoked": req.fingerprint_hex })),
        )
            .into_response(),
        Err(e) => (
...
pub async fn inspect_handler(
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!({ "fingerprint": fingerprint, "revoked": revoked })),
    )
        .into_response()
}
...
fn pk_from_hex(h: &str) -> anyhow::Result<ed25519_dalek::VerifyingKey> {
    ...
    let arr: [u8; 32] = bytes
        .try_into()
        .map_err(|_| anyhow::anyhow!("pk must be 32 bytes"))?;
