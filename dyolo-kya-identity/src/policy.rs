//! YAML/JSON policy-as-code for dyolo-kya.
//!
...
//! Rust. The document is parsed and compiled into a [`dyolo_kya::DelegationPolicy`]
...
///
/// Compiles into a [`dyolo_kya::DelegationPolicy`] via [`PolicyDocument::into_policy`].
///
/// Requires `features = ["serde"]` on `dyolo-kya` for JSON support, and
/// `features = ["policy"]` on `dyolo-kya-identity` for YAML support.
#[derive(Debug, Clone)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
...
impl PolicyDocument {
    ...
    pub fn validate(&self) -> Result<(), Vec<String>> {
        ...
        if let Some(depth) = self.max_chain_depth {
            if depth == 0 { errors.push("max_chain_depth must be > 0".into()); }
        }
        if let Some(ttl) = self.max_ttl_secs {
            if ttl == 0 { errors.push("max_ttl_secs must be > 0".into()); }
        }
        for (i, cert) in self.certs.iter().enumerate() {
            if cert.delegate_pk_hex.len() != 64 || hex::decode(&cert.delegate_pk_hex).is_err() {
                errors.push(format!("Invalid delegate_pk_hex in cert template {} for policy '{}'", i, self.name));
            }
            if cert.ttl_seconds == 0 {
                errors.push(format!("cert template {} ttl_seconds must be > 0", i));
            }
            if cert.intents.is_empty() {
                errors.push(format!("cert template {} must specify at least one intent", i));
            } else {
                ...
                for intent in &cert.intents {
                    ...
                    if intent.name.starts_with("trad.") {
                        errors.push(format!("cert template {} intent '{}' looks like a typo of 'trade.'", i, intent.name));
                    }
                }
            }
        }
    ...
    /// [`dyolo_kya::DyoloChain::authorize_with_options`].
    ...
    ///
    /// Requires the `policy` Cargo feature on `dyolo-kya-identity`.
    #[cfg(feature = "policy")]
